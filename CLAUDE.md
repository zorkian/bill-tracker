...
1. Back up first: `npx wrangler d1 export sm-bill-tracker --remote --output=backups/pre-migration-$(date +%Y%m%d).sql`
2. Apply: `npx wrangler d1 execute sm-bill-tracker --remote --file=src/db/migrations/NNN-name.sql`
3. Verify with a spot-check query: `npx wrangler d1 execute sm-bill-tracker --remote --command="SELECT count(*) FROM bills"`
...
- Never use `npx wrangler d1 execute --remote` without backing up first
